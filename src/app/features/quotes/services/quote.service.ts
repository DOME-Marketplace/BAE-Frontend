...
import { Observable, forkJoin, map, of, throwError, catchError } from 'rxjs';
...
export class QuoteService {
  ...
  private get documentHttpOptions() {
    return {
      headers: new HttpHeaders({
  ...
  downloadAttachment(quote: Quote): Observable<void> {
    ...
    return this.http.get<DocumentSpecificationResponse>(
      ...
      this.documentHttpOptions
    ).pipe(
      map(documentSpecification => {
        if (!this.isDocumentSpecificationResponse(documentSpecification)) {
          throw new Error('Document API response is not JSON. Check documentApi/proxy configuration.');
        }
        ...
        const documentDownloadUrl = documentAttachment
          ? this.getAttachmentDownloadUrl(documentAttachment)
          : null;

        if (documentDownloadUrl) {
          this.startAttachmentDownload(documentDownloadUrl, documentFilename);
          console.log(`Started document attachment download: ${documentFilename}`);
          return;
        }
        ...
        if (this.isS3Reference(documentAttachment.content)) {
          throw new Error('Document attachment content is an S3 reference, not downloadable file content.');
        }
      ...
      catchError(error => throwError(() => this.mapDocumentDownloadError(error)))
    );
  }
  ...
  private isS3Reference(value?: string): boolean {
    return /^s3ref:/i.test(value?.trim() || '');
  }

  private isDocumentSpecificationResponse(value: unknown): value is DocumentSpecificationResponse {
    return Boolean(value && typeof value === 'object' && !Array.isArray(value));
  }

  private mapDocumentDownloadError(error: unknown): Error {
    if (error instanceof HttpErrorResponse &&
        error.status === 200 &&
        (error.error instanceof SyntaxError || error.message.includes('during parsing'))) {
      return new Error('Document API response is not JSON. Check documentApi/proxy configuration.');
    }

    return error instanceof Error
      ? error
      : new Error('Error downloading attachment.');
  }
